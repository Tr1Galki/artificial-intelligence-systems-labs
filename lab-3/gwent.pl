...
fraction_cards(skellige, Deck) :- Deck = [fucusya, greatsword, greatsword].
fraction_cards("scoia'tael", Deck) :- Deck = [dana, saskia].
fraction_cards(nilfgard, Deck) :- Deck = [torres, sergeant, kingslayer, sergeant, kingslayer].
fraction_cards(monsters, Deck) :- Deck = [larva, fleder, grott, larva, fleder].
fraction_cards(northern_realms, Deck) :- Deck = [demovend, draug].
fraction_cards(neutral, Deck) :- 
	Deck = [geralt, roach, ciri, yennefer, triss, regis, "renfri band", "renfri band", renfri].

fraction_default_deck(skellige, Deck) :- Deck = [fucusya, greatsword, greatsword, geralt, roach].
fraction_default_deck("scoia'tael", Deck) :- Deck = [dana, saskia, ciri, yennefer, triss, "renfri band"].
fraction_default_deck(nilfgard, Deck) :- Deck = [torres, sergeant, kingslayer, sergeant, kingslayer].
fraction_default_deck(monsters, Deck) :- Deck = [larva, fleder, grott, fleder, larva, regis].
fraction_default_deck(northern_realms, Deck) :- Deck = [demovend, draug, renfri, "renfri band", "renfri band"].
fraction_default_deck(neutral, Deck) :- 
	Deck = [geralt, roach, ciri, yennefer, triss, regis, "renfri band", "renfri band", renfri].

% input example "I prefer nilfgard fraction. Also I want to have more than 10 cards" 
...
parse(Input, _) :-
	...
	Length < 3,
	!,
	nl, write("Too few words."),
	fail.
parse(Input, Deck) :- 
	length(Input, Length),
	Length < 12,
	nth0(2, Input, Fraction_word),
	term_string(Fraction, Fraction_word),
	fraction(Fraction),
	!,
	nl, write("You chose "), write(Fraction), write(" fraction"),
	fraction_default_deck(Fraction, Deck).
parse(Input, _) :- 
	length(Input, Length),
	Length < 12,
	nth0(2, Input, Fraction_word),
	...
	\+ fraction(Fraction),
	!,
	nl, write("Fraction "), write(Fraction), write(" incorrect"),
	false.
parse(Input, Deck) :- 
	length(Input, Length),
	Length < 15,
	...
	term_string(Fraction, Fraction_word),
	fraction(Fraction),
	nth0(11, Input, Cards_word),
	term_string(Cards, Cards_word),
	number(Cards),
	!,
	nl, write("You chose "), write(Fraction), write(" fraction "),
	write("and at least "), write(Cards), write(" cards"),
	get_deck(Fraction, Cards, Deck).
parse(Input, _) :- 
	length(Input, Length),
	Length < 15,
	write("Incorrect fraction or card numbers"),
	!,
	false.


get_deck(Fraction, Cards_count, Deck) :-
	fraction_cards(Fraction, Deck),
	length(Deck, Length),
	Length > Cards_count,
	nl, write("Your recommended deck:").
get_deck(Fraction, Cards_count, Increased_deck) :-
	fraction_cards(Fraction, Deck),
	length(Deck, Length),
	Length < Cards_count,
	fraction_cards(neutral, Neutral_deck),
	increase_deck(Deck, Neutral_deck, Cards_count, Increased_deck),
	nl, write("Your recommended deck:").

increase_deck(_, [], _, _) :- 
	!,
	nl, write("You want too many cards to deck"),
	false.
increase_deck(Deck, [Card|_], Cards_count, Result_deck) :-
	append(Deck, [Card], Increased_deck),
	length(Increased_deck, Length),
	Length >= Cards_count,
	!,
	Result_deck = Increased_deck.
increase_deck(Deck, [Card|T], Cards_count, Result_deck) :-
	append(Deck, [Card], Increased_deck),
	length(Increased_deck, Length),
	Length < Cards_count,
	!,
	increase_deck(Increased_deck, T, Cards_count, Result_deck).

read_input(WordsList) :- 
	read(Input),
